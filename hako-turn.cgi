...
sub CivReqestEvent {
    ...
    for ($i = 0; $i < $HislandNumber; $i++) {
        ...
        if (   ($island->{'predelete'})
            || (   ($island->{'id'} > $HPlayableIslandMaxID)
                || ($island->{'BF_Flag'}))) {

            CivReqestEventReset($island);
            next;
        }
...
sub doTrade {
    ...
    if (   ($data->{'check1'} == 1)
        ...
        my ($island_pay_m , $island_receive_m);

        # お金
        if ($data->{'payside'}) {
            $island_pay_m = $island1;
            $island_receive_m = $island2;
        }
        else {
            $island_pay_m = $island2;
            $island_receive_m = $island1;
        }
        ...
        my ($island_pay_obj , $island_receive_obj);

        # オブジェ
        if ($data->{'objside'}) {

            $island_pay_obj = $island1;
            $island_receive_obj = $island2;
        }
        else {
            $island_pay_obj = $island2;
            $island_receive_obj = $island1;
        }

        {
            my ($obj_check) = TradeValtoId($data->{'obj'});

            if ($obj_check eq NULL) {
                logOut("取引データが正しく読めなかったので、取引ができませんでした。：chk:$obj_check" , $island1_id);
                logOut("取引データが正しく読めなかったので、取引ができませんでした。：chk:$obj_check" , $island2_id);
                return;
            }
        }
        ...
        {
            my ($obj_ele) = GetTradeObjElement($data->{'obj'});
            my ($obj_name) = GetTradeObjName($data->{'obj'});

            if ($island_pay_obj->{$obj_ele} < $data->{'obj_num'}) {

                logOut("輸出量が不足しているため、取引ができませんでした。：$obj_name" , $island_pay_obj->{'id'});
                logOut("相手の輸出量が不足しているため、取引ができませんでした。：$obj_name" , $island_receive_obj->{'id'});
                return ;
            }

            if (GetTradeObjKind($data->{'obj'}) == $TradeKindTEST) {

                logOut("取引するものがテスト向けのため、実施できませんでした。：$obj_name" , $island_pay_obj->{'id'});
                logOut("取引するものがテスト向けのため、実施できませんでした。：$obj_name" , $island_receive_obj->{'id'});
                return ;
            }

            if (GetTradeObjKind($data->{'obj'}) == $TradeKindFood) {

                food_product_plus($island_pay_obj , $island_pay_obj->{$obj_ele} , -$data->{'obj_num'});
                food_product_plus($island_receive_obj , $island_pay_obj->{$obj_ele} , +$data->{'obj_num'});

                $island_pay_m->{'money'} -= $data->{'money'};
                $island_receive_m->{'money'} += $data->{'money'};

                my ($pay) = "$data->{'money'}$HunitMoney";

                logOut("取引が行われました。：$obj_name" , $island1_id);
                logOut("取引が行われました。：$obj_name" , $island2_id);
            }
            else {
                logOut("エラーです" , $island1_id);
                logOut("取引が行われました。：$obj_name" , $island2_id);
...
#       logPropaganda($id, $name, $comName);
    ...
    if (   ($kind == $HcomPrepare)
        ...
        my ($seaCount) = countAround($land, $x, $y, 7, @Hseas);         # 周りに陸があるかチェック
        if (   ($seaCount == 7)
            && ($kind != $HcomReclaim2)
            && ($kind != $HcomSeki)) {
        ...
        if (   ($kind == $HcomReclaim)
            ...
            $land->[$x][$y] = $HlandMinato;                                 # 目的の場所を港にする
            $landValue->[$x][$y] = 0;
            $landValue2->[$x][$y] = 0;
            $landValue3->[$x][$y] = 0;
        ...
        if (0) {
            ...
            unless (   ($island->{'kyu'} > 1)
                ...
                if ($value > 0) {
                    ...
                    $island->{'money'} += $value;
                    logOilMoney($id, $name, $lName, $this_pos, "$value$HunitMoney", "収益");            # 収入ログ
                }
            }
            ...
            if (random(100) < $DepletionPar) {                                                  # 枯渇判定
                # 枯渇
            ...
            elsif ($mKind == $Mons_KaminariInora) {

                my ($num) = random(3) + 1;
                logKaminariCall($id, $name, $mName, "($x, $y)");
